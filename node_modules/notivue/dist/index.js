...
function createQueue() {
  return {
    ...
    update(id, newOptions) {
      ...
      if (entry)
        Object.assign(entry, newOptions);
    },
    remove(id) {
...
function createAnimations(config, items, queue, elements) {
  return {
    ...
    playClearAll() {
      ...
      if (!clearAll || this.isReducedMotion.value)
        return onAnimationend();
    ...
    updatePositions({ isImmediate = false } = {}) {
      ...
      for (const el of elements.getSortedItems()) {
        ...
        const item = items.get(id);
        if (!el || !item || item.animationAttrs.class === leaveClass)
          continue;
        items.update(id, {
          positionStyles: {
...
function createTimeouts(items, animations) {
  return {
    ...
    pause() {
      if (items.length === 0 || this.isStreamPaused.value)
        return;
      this.setStreamPause();
          ...
          animationAttrs: {
            ...
            onAnimationend() {
              items.update(entry.id, {
                animationAttrs: { class: "", onAnimationend: void 0 }
              });
            }
          },
...
function createNotivue(pluginConfig = {}) {
  return {
    install(app) {
      ...
      const instance = runInstance(store, push2);
...
function createInstance(startOnCreation) {
  ...
  function runInstance(store, push2) {
...
function useResizeListObserver(elements, onSizeChange) {
  ...
  watch(
    ...
    (el) => {
      if (el.length > 0)
        el.forEach((el2) => resizeObserver == null ? void 0 : resizeObserver.observe(el2));
    },
    { flush: "post" }
  );
  onBeforeUnmount(() => {
          ...
          (openBlock(true), createElementBlock(Fragment, null, renderList(unref(items).entries.value, (item, i) => {
            ...
            return openBlock(), createElementBlock("li", {
              ...
              item.ariaLiveOnly ? (openBlock(), createBlock(_sfc_main$g, {
                key: 0,
                item
              }, null, 8, ["item"])) : (openBlock(), createElementBlock("div", mergeProps({ key: 1 }, item.animationAttrs, {
                ...
                renderSlot(_ctx.$slots, "default", normalizeProps(guardReactiveProps(unref(getSlotItem)(item))))
              ], 16, _hoisted_3$6))
            ], 12, _hoisted_2$9);
          }), 128))
...
const _sfc_main$d = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    const props = __props;
    function onPush(e) {
      const notification = push$1[e.detail.type](e.detail);
      window.dispatchEvent(
    ...
    const onClearAll = () => push$1.clearAll();
    const onDestroyAll = () => push$1.destroyAll();
    const events = [
      ["notivue:push", onPush],
...
const _sfc_main$c = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    function isPointerInside(e) {
      if (!elements.root.value || !itemRef.value)
        return false;
    ...
    function onPointerDown(e) {
      ...
      if (e.button !== 0)
        return;
      if (state.isPressed || state.isClearing)
        return;
      if (exclude.value) {
...
function useLastFocused() {
  ...
  function onFocusCapture(e) {
    ...
    if (isValidTarget && ((_a = stream.value) == null ? void 0 : _a.contains(e.target)))
      return;
    if (isValidTarget)
  ...
  setup(__props) {
    ...
    function onAllowedStreamNavigation(e) {
      if (!e.shiftKey && e.key === "Tab" && candidateContainers.value.length > 0) {
        e.preventDefault();
        if (hasNeverTabbedStream)
          hasNeverTabbedStream = false;
        onStreamEnter();
    ...
    watch(
      ...
      (newUnqualified) => {
        if (!config.enqueue.value || !timeouts.isStreamFocused.value)
          return;
      ...
      (stream, _, onCleanup) => {
        function onStreamFocusOut(e) {
          ...
          if (!isKeyboardFocus.value)
            return;
          if (isManualLeave)
            return isManualLeave = false;
          if (!(stream == null ? void 0 : stream.contains(e.relatedTarget))) {
            onStreamLeave();
          }
        }
        if (stream)
          stream.addEventListener("focusout", onStreamFocusOut);
        onCleanup(() => {
          if (stream)
            stream.removeEventListener("focusout", onStreamFocusOut);
        });
      },
      { immediate: true, flush: "post" }
    );
